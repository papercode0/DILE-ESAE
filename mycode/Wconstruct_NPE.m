function [WW] = Wconstruct_NPE(options, data)
...
%     M = sparse(M);
    ...
    for ii=1:nSmp
        ...
        C = z*z';                                        % local covariance
        C = C + eye(size(C))*tol*trace(C);                   % regularlization
        if norm(C,2) == 0                               %邻域全等于它自身
            W(:,ii) = ones(options.k,1); 
        else
            W(:,ii) = C\ones(options.k,1);                           % solve Cw=1  A\B表示A的逆*B
        end     
        %W(:,ii) = C\ones(options.k,1);                           % solve Cw=1
        W(:,ii) = W(:,ii)/sum(W(:,ii));                  % enforce sum(w)=1
        
        WW(ii, neighborhood(ii,:)) =  W(:,ii)';
    end
